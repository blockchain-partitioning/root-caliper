...
function installChaincode(org, chaincode) {
    ...
    channel.addOrderer(client.newOrderer(ORGS.orderer.url, {
        'pem': caroots,
...
function instantiateChaincode(chaincode, endorsement_policy, upgrade) {
    ...
    return Client.newDefaultKeyValueStore({
        ...
        eh.setPeerAddr(ORGS[userOrg][eventPeer].events, {
            pem: Buffer.from(data).toString(),
            ...
            eventhubs.forEach((eh) => {
                let txPromise = new Promise((resolve, reject) => {
                    ...
                    eh.registerTxEvent(deployId.toString(), (tx, code) => {
                        ...
                        if (code !== 'VALID') {
                            util_1.default.log('The chaincode ' + type + ' transaction was invalid, code = ' + code);
                            reject();
                        }
                        else {
                            util_1.default.log('The chaincode ' + type + ' transaction was valid.');
                            resolve();
                        }
                    });
                });
        ...
        for (let i in channelConfig.organizations) {
            ...
            if (peers.length === 0) {
                ...
                peers.forEach((peerInfo) => {
                    ...
                    let peer = client.newPeer(peerInfo.requests, {
                        pem: Buffer.from(data).toString(),
                    ...
                    if (org === userOrg) {
                        let eh = client.newEventHub();
                        eh.setPeerAddr(peerInfo.events, {
                            pem: Buffer.from(data).toString(),
                            'ssl-target-name-override': peerInfo['server-hostname'],
...
function invokebycontext(context, id, version, args, timeout) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let invokeStatus = {
            ...
            result: null,
            verified: false,
            error_flags: constant_1.TxErrorEnum.NoError,
        ...
        try {
            ...
            try {
                proposalResponseObject = yield channel.sendTransactionProposal(proposalRequest, timeout * 1000);
                invokeStatus.time_endorse = Date.now();
            }
            catch (err) {
                invokeStatus.time_endorse = Date.now();
                invokeStatus.error_flags |= constant_1.TxErrorEnum.ProposalResponseError;
                invokeStatus.error_messages[constant_1.TxErrorIndex.ProposalResponseError] = err.toString();
            ...
            for (let i in proposalResponses) {
                ...
                if (proposal_response.response && proposal_response.response.status === 200) {
                    ...
                    invokeStatus.error_flags |= constant_1.TxErrorEnum.BadProposalResponseError;
                    invokeStatus.error_messages[constant_1.TxErrorIndex.BadProposalResponseError] = err.toString();
            ...
            if (newTimeout < 10000) {
                util_1.default.log('WARNING: timeout is too small, default value is used instead');
                newTimeout = 10000;
            }
            const eventPromises = [];
            eventHubs.forEach((eh) => {
                eventPromises.push(new Promise((resolve, reject) => {
                    ...
                    eh.registerTxEvent(txId, (tx, code) => {
                        ...
                        if (code !== 'VALID') {
                            ...
                            invokeStatus.error_flags |= constant_1.TxErrorEnum.BadEventNotificationError;
                            invokeStatus.error_messages[constant_1.TxErrorIndex.BadEventNotificationError] = err.toString();
                        ...
                        invokeStatus.error_flags |= constant_1.TxErrorEnum.EventNotificationError;
                        invokeStatus.error_messages[constant_1.TxErrorIndex.EventNotificationError] = err.toString();
                        resolve();
                    });
                }));
            });
            ...
            try {
                broadcastResponse = yield channel.sendTransaction(transactionRequest);
            }
            catch (err) {
                ...
                invokeStatus.error_flags |= constant_1.TxErrorEnum.OrdererResponseError;
                invokeStatus.error_messages[constant_1.TxErrorIndex.OrdererResponseError] = err.toString();
            }
            ...
            if (broadcastResponse && broadcastResponse.status === 'SUCCESS') {
                ...
                invokeStatus.error_flags |= constant_1.TxErrorEnum.BadOrdererResponseError;
                invokeStatus.error_messages[constant_1.TxErrorIndex.BadOrdererResponseError] = err.toString();
            ...
            if (!invokeStatus.verified) {
                ...
                util_1.default.log('Failed to complete transaction [' + txId.substring(0, 5) + '...]: every eventhub connection closed');
            }
            else {
            ...
            invokeStatus.status = 'failed';
            util_1.default.log('Failed to complete transaction [' + txId.substring(0, 5) + '...]:' + (err instanceof Error ? err.stack : err));
        }
        invokeStatus.time_final = Date.now();
        return invokeStatus;
    });
}
exports.invokebycontext = invokebycontext;
/**
...
function querybycontext(context, id, version, name) {
    ...
    return channel.queryByChaincode(request)
        ...
        .catch((err) => {
        util_1.default.log('Query failed, ' + (err.stack ? err.stack : err));
        invoke_status.time_final = Date.now();
