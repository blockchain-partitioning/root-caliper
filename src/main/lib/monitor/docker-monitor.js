...
const monitor_interface_1 = require("./monitor-interface");
...
function findContainers() {
    console.log("Retreiving container ID's");
 ...
 */
class DockerMonitor extends monitor_interface_1.default {
    /**
     * Constructor
    ...
    start() {
        ...
        return this.hasContainters.then(() => {
            console.log("Retrieved container ID's");
            let self = this;
            /**
                        ...
                        if (self.containers[i].remote === null) { // local
                            ...
                            if (cpuDelta > 0.0 && sysDelta > 0.0) {
                                if ((stat.cpu_stats.cpu_usage.hasOwnProperty('online_cpus') || stat.cpu_stats.cpu_usage.hasOwnProperty('percpu_usage')) && stat.cpu_stats.cpu_usage.percpu_usage !== null) {
                                    self.stats[id].cpu_percent.push(cpuDelta / sysDelta * DockerMonitor.coresInUse(stat.cpu_stats) * 100.0);
                                }
                                else {
            ...
            console.log("Starting to read containers statistics");
    ...
    static coresInUse(cpu_stats) {
        return cpu_stats.online_cpus || DockerMonitor.findCoresInUse(cpu_stats.cpu_usage.percpu_usage || []);
    }
    ...
    stop() {
        ...
        this.containers = [];
        this.stats = { 'time': [] };
        return util_1.default.sleep(100);
    }
    /**
    ...
    createDataDump(label) {
        util_1.default.createDataDump(label, { peers: this.getPeers(), statistics: this.stats });
    }
}
exports.default = DockerMonitor;
//# sourceMappingURL=docker-monitor.js.map